...
def render_sidebar(df: pd.DataFrame, session: dict):
    with st.sidebar:
        ...
        <div style="font-size:0.72rem; color:{TEXT_DIM}; line-height:2">
            <div style="color:{TEXT_PRIMARY};font-weight:600;font-size:0.82rem;font-family:'Rajdhani',sans-serif;">Hardik R Prajapati</div>
            ...
            <div>🎓 B.E. AI&DS — Class of 2026</div>
            <div>📋 Enroll: 220200143051</div>
            <div>🏆 MS Elevate Internship 2026</div>
            <div style="margin-top:6px">
                <a href="https://www.linkedin.com/in/hardik-prajapati-60bb90339/" target="_blank"
                   style="color:{CYAN};text-decoration:none">🔗 LinkedIn</a>
                &nbsp;·&nbsp;
                <a href="https://github.com/HardikPrajapati08" target="_blank"
...
def render_main(df: pd.DataFrame, session: dict, anomaly: dict,
    ...
    # ── Row 1: Live Video Feed + Alert Feed ──────────────────────────────
    col_vid, col_feed = st.columns([2.0, 1.4])
    ...
    # ── Row 2: Priority donut + Heatmap + Anomaly timeline ─────────────────
    col_donut, col_heat, col_anom = st.columns([1.0, 1.2, 1.5])
    ...
    # ── Row 3: Alerts over time + Class distribution + Motion score + Session table ─────
    col_atot, col_cls, col_mot, col_ses = st.columns([1.0, 1.0, 1.0, 1.0])
